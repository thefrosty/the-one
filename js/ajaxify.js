...
(function ($, window, undefined) {
    ...
    $(function () {
        ...
        /* Application Specific Variables */
            contentSelector = '#content,article:first,.entry:first,.post:first',
            $content = $(contentSelector).filter(':first'),
            contentNode = $content.get(0),
            $menu = $('nav').filter(':first'),
            activeClass = 'active selected current',
            activeSelector = '.active,.selected,.current,.current-menu-item,.current_page_item',
            menuChildrenSelector = 'li.menu-item',
        ...
        $.fn.ajaxify = function () {
            ...
            // Ajaxify
            $this.find('a:internal:not(.no-ajaxy, .more-posts a, .bx-prev, .bx-next, a[href^="#"], a[href*="wp-login"], a[href*="wp-admin"], a[target="_blank"])').click(function (event) {
                // Prepare
                var
                    $this = $(this),
                    url = $this.attr('href'),
        ...
        $window.bind('statechange', function() {
            ...
            $.ajax({
                ...
                success: function (data, textStatus, jqXHR) {
                    ...
                    // http://stackoverflow.com/a/21805932
                    var
                        doc = parser.parseFromString(data, "text/html"),
                        docClass = doc.body.getAttribute('class');

                    // Garbage collection, don't need this anymore.
                    parser = doc = null;

                    //Add classes to body
                    $body.prop('class', docClass);
